...
const WEEKDAY_PATTERN = new RegExp(
  `\\b(${WEEKDAYS.join('|')}|mon|tue|tues|wed|thu|thur|thurs|fri|sat|sun)\\b`, 'i',
);
 ...
 * getting it right.
 */
function claimedWeekday(text) {
  const m = WEEKDAY_PATTERN.exec(text ?? '');
  if (!m) return null;
  const word = m[1].toLowerCase();
  const full = ABBREV_TO_FULL[word] ?? word[0].toUpperCase() + word.slice(1);
  return WEEKDAYS.includes(full) ? full : null;
}

function actualWeekday(dateStr) {
 ...
 */
export function validateAttribution(attr, { kind, postedDate, today, text, backdateDays = DEFAULT_BACKDATE, confidenceFloor = 0.6 }) {
  ...
  if (claimed) {
    const actual = actualWeekday(attr.effective_date);
    if (claimed !== actual) {
      return { ok: false, reason: `text says "${claimed}" but ${attr.effective_date} is a ${actual}` };
    }
  }
...
export async function enrichPending(db, cfg, { runClaude = runClaudeCli, now = new Date(), log = () => {} } = {}) {
  ...
  for (const attr of attributions) {
    ...
    const check = validateAttribution(attr, {
      kind: item.kind, postedDate: item.postedDate, today, text: item.text,
      backdateDays, confidenceFloor: cfg.enrich.confidenceFloor ?? 0.6,
    });
    if (!check.ok) {
